...
- **多言語対応** — WordPress 標準の翻訳ファイル（.pot/.po/.mo・JS 翻訳 JSON）に準拠。サイトの言語設定に応じて自動的に翻訳される（同梱: 英語）
...
管理画面はサイトの言語設定（設定 > 一般 > サイトの言語）に応じて自動的に翻訳されます。現在同梱している翻訳は英語（`en_US`）です。ソース言語（未翻訳時のフォールバック）は日本語です。

新しい言語を追加する手順や翻訳ファイルの構成は [SPEC.md](SPEC.md#多言語対応i18n) を参照してください。
...
├── languages/                                    # 翻訳ファイル（.pot/.po/.mo・JS 翻訳 JSON）
├── README.md
├── SPEC.md
├── CLAUDE.md
└── .gitignore
